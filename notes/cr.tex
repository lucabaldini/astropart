...
Let us assume that, in the laboratory frame, the cloud is moving along the $x$-axis and
the test particle (e.g., a proton) is impinging on the hard wall at an angle $\theta$,
measured from the direction of motion of the cloud, as shown in figure~\ref{fig:fermi_accel}.
Note that in our setup the particle is always moving toward negative $x$ values,
so that $\cos\theta < 0$ when the cloud is moving in the negative $x$ direction, i.e.,
for a following collision. The relevant Lorentz transformations involve the particle
energy $E$ and the momentum component $p_x$ parallel to the boost, but since our test
particle is relativistic, we will make our life easier by assuming that
$$
  \abs{p_x c} = \abs{pc \cos\theta} \approx \abs{E \cos\theta},
$$
ans express everything in terms of energy---before and after the collision, and
...
\begin{align}
  \begin{cases}
    E_i' = \gamma E_i (1 + \beta \cos\theta)\\
    E_i' \cos\theta' = \gamma E_i (\cos\theta + \beta).
  \end{cases}
  \tag{Lab $\rightarrow$ Cloud}
\end{align}
(Take a deep breath and make sure you understand the plus sign on the right-hand
side of both equations. The result is consistent with our intuition that the
energy in the cloud reference frame is larger than in the laboratory for head-on
collisions.)

Since the collision is elastic in the cloud reference frame, the energy
