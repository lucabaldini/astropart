...
\subsection{Composition}\label{sec:cr_composition}
...
and the cross-section $\sigma$ of the spallation process
\begin{align*}
  \dv{N_s}{\ell} = \sigma n_\text{ISM} N_p.
\end{align*}
This is more conveniently rewritten in terms of our old acquaintance, the grammage
$\diff{\grammage} = \density_\text{ISM} \diff{\ell} = \ave{m} n_\text{ISM} \diff{\ell}$
\begin{align}
  \dv{N_s}{\grammage} = \frac{\sigma}{\ave{m}} N_p,
\end{align}
where $\ave{m}$ is the average mass of an interstellar medium particle. For every
secondary particle produced, a primary particle is destroyed, so that the rate of
change of the number of primaries is given by
\begin{align*}
  \dv{N_p}{\grammage} = -\frac{\sigma}{\ave{m}} N_p \quad\text{or}\quad
  N_p(\grammage) = N_p(0) \exp\qty(-\frac{\sigma}{\ave{m}} \grammage).
\end{align*}
