...
\section{Propagation of cosmic rays}\label{sec:cr_propagation}
...
\end{align*}
and, if we assume for a second that the number of primaries does not change
significantly in the process, which is true especially at high energies, this can
be readily integrated to yield
\begin{align}
...
traversed by the cosmic rays}, and current measurements concur to indicate a
.

As usual, we shall model the Galaxy as a relatively thin disk of thickness
$h_\text{disk} \approx 100$~pc and average density $n_\text{disk} \approx 1$~cm~$^{-3}$,
surrounded by a much thicker diffusion halo.
