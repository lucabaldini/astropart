...
plt.axhline(Ek, color='black', ls='dashed')
plt.text(r1, Ek + 0.02, '$E$')
setup_ca(xlabel='$r$ [a.u]', ylabel='$U(r)$ [a. u.]', xticks=(1., r1), yticks=(-0.5, 0., 1.))
plt.gca().set_xticklabels(('$r_0$', "$r_1$"))
...
plt.text(0.3, 5.e10, f'pp ({100. * nusun.FLUX_PP / nusun.FLUX_TOTAL:.1f}%)')
_plot_line(nusun.ENERGY_PEP, nusun.FLUX_PEP, **pp_fmt)
plt.text(nusun.ENERGY_PEP, 2. * nusun.FLUX_PEP, f'pep ({100. * nusun.FLUX_PEP / nusun.FLUX_TOTAL:.1f}%)')
nusun.spec_hep.plot(**pp_fmt)
plt.text(10, 1.5e2, f'hep', ha='right')
for energy, yield_ in zip(nusun.ENERGIES_BE7, nusun.YIELDS_BE7):
    flux = nusun.FLUX_BE7 * yield_
    _plot_line(energy, flux, **pp_fmt)
    plt.text(energy, 1.5 * flux, f'${{}}^7$Be ({100. * flux / nusun.FLUX_TOTAL:.1f}%)',
        ha='right' if energy < 0.5 else 'left')
nusun.spec_b8.plot(**pp_fmt)
plt.text(7., 1.e5, f'${{}}^8$B ({100. * nusun.FLUX_B8 / nusun.FLUX_TOTAL:.3f}%)')
