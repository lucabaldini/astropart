...
The detection principle is interesting. Water is made of hydrogen and oxygen, and the
former would be a sensible target for antineutrinos (which, unfortunately, are basically
not produced in the Sun) through the reactions
\begin{align*}
  \bar{\nu}_e + p \rightarrow n + e^+,
\end{align*}
which has no equivalent for neutrinos. Capture on oxygen
\begin{align*}
  \nu_e + \ce{^{16}O} \rightarrow \ce{^{16}F} + e^-,
\end{align*}
is technically possible, but since oxygen is a very tightly bound nucleus, the reaction
has a very high kinematic threshold ($\sim 15$~MeV) and is therefore basically
irrelevant in this context. All we are left with is the elastic neutrino scattering
on the atomic electrons\sidenote{Technically elastic scattering on the protons is also
a thing, but detecting a protont with a MeV energy is much more challenging.}. This is
qualitatively different from the radiochemical experiments in that, while the latter
are only sensitive to $\nu_e$, \emph{water \cherenkov\ experiments are sensitive to all
three generations of neutrinos} via the reactions:
\begin{align*}
