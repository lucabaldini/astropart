...
The main mechanism generating energy in the Sun is the fusion of $H$ into $He$,
and the basic reaction by which the Sun shines is the conversion of \emph{four}
...
\end{align*}
The $Q$ value of this reaction can be readily calculated noting that it can be
simply rewritten as a function of the total binding energy of the $\ce{^4He}$
nucleus, which we know is
\begin{align*}
...
mass difference ($m_n - m_p \approx 1.293$~MeV) and the electron mass
...
This is a complex process, going under the name of $pp$ chain, which proceeds
via a chain of reactions (more on this in section~\ref{sec:nu_production_sun})
but the key point is that, owing to the conservation of the leptonic number,
proceeds with the creation of exactly two neutrinos for each
reaction\sidenote{What we are doing here is essentially to transform two out of
four protons in neutrons,
$$
p + e^- \rightarrow n + \nu_e,
$$
which is the inverse of the neutron beta decay, and produce one neutrino along
with each neutron.}.
